...
Note: `a2rchi` settings live in `src/a2rchi/a2rchi-default-settings.yaml` and are seeded into Postgres at deployment; runtime services read settings from Postgres only. You can override defaults by providing an `a2rchi` section in the same config file. Static config is used for deployment-time defaults and is also stored in Postgres for runtime reads (including `data_manager`). Runtime loaders use `load_runtime_config`/`load_runtime_*` for deployment config and `load_a2rchi_settings` for agent settings to keep the split explicit. Per-agent descriptions live under `a2rchi.pipeline_map.<pipeline>.agent_description`. Config files are plain YAML; Jinja templating is not supported.
